...
export const SuccessView: React.FC<SuccessViewProps> = ({ order, onNavigate }) => {
    ...
    return (
      <div className="bg-[#FAF6F0] min-h-screen flex flex-col items-center justify-center p-8">
        ...
        <button
          ...
          className="bg-[#C4601A] text-white px-6 py-2.5 rounded-full text-sm font-semibold hover:bg-[#FFF0E8]"
        >
          Return to Shop
        </button>
      </div>
  ...
  return (
    <div id="page-success" className="bg-[#FAF6F0] min-h-screen">
      ...
      <div className="flex flex-col items-center justify-start p-6 pt-8 pb-10 max-w-[620px] mx-auto">
      ...
      <h1 className="success-title font-serif text-3xl md:text-[38px] font-bold text-[#111111] mb-1.5 text-center">
        Order Confirmed!
      </h1>
      ...
      <div className="success-order-id bg-white border border-[#E8E0D5] rounded-xl px-5 py-3 text-sm text-[#111111] font-bold mb-4 w-full text-center shadow-xs">
        Order ID:{' '}
        <strong id="success-order-id" className="text-[#C4601A] font-extrabold">
          #{order.id}
        </strong>
      </div>
      ...
      <div id="success-items-card" className="success-card bg-white rounded-xl p-5 border border-[#E8E0D5] w-full shadow-xs mb-4">
        <h3 className="success-card-title font-serif text-base font-bold text-[#111111] mb-4">
          Order Summary
        </h3>
        ...
        <div id="success-items-list" className="flex flex-col gap-2.5">
          {(order.items || []).map((i, idx) => (
            <div key={idx} className="success-item flex justify-between text-xs">
              <span className="success-item-name text-[#111111] font-medium">
                {i.name || 'Saree'} <span className="text-[#333333] font-bold">x{i.qty || 1}</span>
              </span>
              <span className="success-item-price font-bold text-[#111111]">
        ...
        <div className="success-item flex justify-between text-sm items-center">
          ...
          <span className="success-item-price text-base font-extrabold text-[#C4601A]" id="success-total">
            ₹{(order.total || 0).toLocaleString('en-IN')}
          </span>
        </div>

        <div className="success-item flex justify-between text-xs items-center mt-3 text-[#333333] font-semibold">
          <span>Payment Info</span>
          <span className="font-bold text-emerald-700 block text-right" id="success-payment-method">
            {(order.method || 'Processing')} ✓
          </span>
        </div>
      </div>

      <div className="success-delivery text-[#222222] text-xs font-bold text-center mb-6">
      ...
      <button
        onClick={() => onNavigate('home')}
        className="continue-btn w-full border-1.5 border-[#C4601A] text-[#C4601A] hover:bg-[#C4601A]/4 py-3.5 rounded-xl text-sm font-semibold active:scale-99 transition-all cursor-pointer text-center bg-transparent"
      >
        Continue Shopping
      </button>
      </div>
